...
void loop() {
  ...
  if(select_state && millis() > lastDebounceTime_dir_but){
  ...
  if(select_city && millis() > lastDebounceTime_dir_but){
    city_count = choose_city(city_count);
    city_print(state_count, city_count);
    lastDebounceTime_dir_but = millis() + debounceDelay;
  }
...
void city_print(int st_count, int ct_count){
  int stat_pos = (ct_count/4)*4;
  int m = state_city_num[st_count]-1;
  for(int i = stat_pos;i<stat_pos+4;i++){
    if(i<= m){
      if(i==ct_count){
        lcd.print("*");
      }else{
        ...
        //lcd_print_char(states[st_count][i]);
      //if(i == stat_pos+4) lcd.setCursor(0, 1);
    }
    //if(i== 54) lcd.print("                ");
  }
...
void lcd_print_char(char city_name[]){
  int len = strlen(city_name);
  for(int i = 0;i<len;i++){
    lcd.print(city_name[i]);
  }
}
